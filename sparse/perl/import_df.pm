...
#    my %default = (
    my %colmeta = (
        mdctn_rxnorm => "mdctn_rxnorm_meta.csv",
        mdctn_gene_meta => "mdctn_gene_meta.csv",
...
#    my %colmeta = (%default, @_);
    ...
    sub colname {
        ...
        foreach my $k (keys %colnames_dict) { #xxx the order here is nondeterministic
            ...
            if ($x =~ m/^$k/) {
                ...
                return @ret; # xxx does the memory pointed to by this reference become deallocated when the function returns?
            }
        }
        return $x;
    }
    ...
    my $ref=import_headers($filemeta);
    my @colnames = map(colname($_), @${ref}); # colnames is reference to an array
    #xxx the order above is nondeterministic because colname uses a foreach
    # under what circumstances does colnames be anything but a vector? This will affect paths through parse_row
...
TRUE if the Input object (line) is currently at an "entry separator", after skipping it
FALSE otherwise
...
=cut
sub parse_entry {
    my ($inp, $row, $names) = @_; # xxx looks like "$names" is actually just a string?
    if(scalar @$names > 1) {
        if ($inp->curr() eq '\"'){
            $inp->skip("\"");
            ...
            foreach my $inx (@{$indices}) {
                ${$row}{${$names}[${$indices}[$inx]]} = ${$elements}[$inx];
            }
            $inp->skip("\"");
        } elsif ($inp->curr() eq "(") {
            my %entry = parse_sparse_array($inp);
            ...
            my $elements = $entry{'elements'};
            foreach my $inx (@$indices) {
                ${$row}{${$names}[${$indices}[${$indices}[$inx]-1]]} = ${$elements}[$inx];
            }
        } else {
            my %entry = undef; # this seems unecessary
        }

        # print("list entry: " + str(entry))
    } else {
        my $s = parse_unquoted_string($inp);
        # print("simple entry: " + $s + "[" + ${$names}[0] + "]")
        ${$row}{${$names}[0]} = $s;
    }
}
...
=cut
sub wrap {
    return shift; # I think perl takes care of this casting xxx
...
#       xxx maybe use Statistics::R to do this
    ...
    while(<FP>) { $callback2->(parse_row($_, $colnames)); }
